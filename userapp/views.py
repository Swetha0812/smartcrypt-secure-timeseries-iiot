...
def user_register(request):
    if request.method == "POST" and 'profile' in request.FILES:
        ...
        if UserRegisterModel.objects.filter(user_email=email):
            ...
            UserRegisterModel.objects.create(user_name=name,user_email=email,user_contact=contact,user_state=state,user_dob=birth,user_profile=profile,user_password=password,otp=otp)
...
def user_file_upload(request):
    ...
    if request.method == "POST" and 'file' in request.FILES:
        if user_rights.user_upload == "YES":
            ...
            messages.error(request,'you did not have the permission to upload this file')
